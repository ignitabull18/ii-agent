...
Saves the output as an MP3 file in the workspace. Available voices: {', '.join(AVAILABLE_VOICES)}."""
    ...
    def _check_ffmpeg(self):
        try:
            subprocess.run(["ffmpeg", "-version"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
             ...
             print("You can typically install it via your system's package manager (e.g., `apt install ffmpeg`, `brew install ffmpeg`).")
    ...
    def run_impl(
        ...
        system_prompt = tool_input.get("system_prompt", "You are a helpful voice assistant.")
        ...
        output_mp3_path = self.workspace_manager.workspace_path(Path(relative_output_filename))
        ...
        temp_wav_path = self.workspace_manager.workspace_path(Path("uploads") / temp_wav_filename) # Store temp in uploads
        ...
        try:
            ...
            try:
                subprocess.run(
                    [
                        ...
                        "-i", str(temp_wav_path),
                        "-acodec", "libmp3lame",
                        "-b:a", "64k", # Bitrate for compression
                        str(output_mp3_path),
                    ],
            ...
            try:
                os.remove(temp_wav_path)
                 ...
                 print(f"Warning: Could not delete temporary WAV file {temp_wav_path}: {e}") # Non-fatal warning
            ...
            return ToolImplOutput(
             ...
             if temp_wav_path.exists(): os.remove(temp_wav_path) # Clean up temp file
             return ToolImplOutput(
