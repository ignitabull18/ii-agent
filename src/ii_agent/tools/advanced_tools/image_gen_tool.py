...
import vertexai
from vertexai.preview.vision_models import ImageGenerationModel # Use preview for Imagen 3
...
class ImageGenerateTool(LLMTool):
    ...
    description = f"""Generates an image based on a text prompt using Google's Imagen 3 model via Vertex AI.
The generated image will be saved to the specified local path in the workspace as a PNG file."""
    ...
    def __init__(self, workspace_manager: WorkspaceManager):
        ...
        try:
            ...
            self.model = ImageGenerationModel.from_pretrained("imagen-3.0-generate-002") # As per snippet
    ...
    def run_impl(
        ...
        if seed is not None:
            ...
            if add_watermark:
                print("Warning: 'seed' is provided, 'add_watermark' will be ignored (or set to False).")
        ...
        try:
            ...
            if generate_params["number_of_images"] > 1:
                print(f"Warning: Requested {generate_params['number_of_images']} images, but tool currently saves only the first.")
            
            images[0].save(location=str(local_output_path), include_generation_parameters=False) # include_generation_parameters=False as per snippet
            ...
            return ToolImplOutput(
                ...
                {
                    "success": True, "output_path": relative_output_filename, 
                    "url": output_url
                }
            )
