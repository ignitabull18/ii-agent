...
class ListHtmlLinksTool(LLMTool):
    ...
    def _extract_links_from_file(self, file_path: Path) -> Set[str]:
        links = set()
        if not file_path.exists() or not file_path.is_file() or file_path.suffix.lower() != ".html":
            return links
        
        html_content = file_path.read_text(errors='ignore')
        # Basic regex, consider BeautifulSoup for robustness
        for match in re.finditer(r'<a\s+(?:[^>]*?\s+)?href="([^"]*)"', html_content, re.IGNORECASE):
            href = match.group(1)
    ...
    def run_impl(
        ...
        if not ws_path.exists():
            return ToolImplOutput(f"Error: Path not found: {relative_path_str}", f"Path not found: {relative_path_str}", {"success": False})

        if ws_path.is_file():
            if ws_path.suffix.lower() == ".html":
                all_found_links.update(self._extract_links_from_file(ws_path))
            else:
                return ToolImplOutput(f"Error: Specified path '{relative_path_str}' is not an HTML file.", "Path is not HTML", {"success": False})
        elif ws_path.is_dir():
            for item in ws_path.rglob('*.html'): # Recursively find all HTML files
                if item.is_file():
                    all_found_links.update(self._extract_links_from_file(item))
        else:
            return ToolImplOutput(f"Error: Path is neither a file nor a directory: {relative_path_str}", "Invalid path type", {"success": False})
