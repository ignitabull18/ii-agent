...
class Browser:
	...
	def _init_browser(self):
		...
		if self.playwright_browser is None:
			if self.config.cdp_url:
				...
				while True:
					try:
						self.playwright_browser = self.playwright.chromium.connect_over_cdp(
							self.config.cdp_url,
							timeout=2500,
						)
						break
	...
	def _update_state(self) -> BrowserState:
		...
		def get_stable_state():
			...
			interactive_elements_data = self.get_interactive_elements(screenshot_b64, detect_sheets)
	...
	def get_interactive_elements(self, screenshot_b64: str, detect_sheets: bool = False) -> InteractiveElementsData:
		"""
		...
		if self.detector is not None:
			...
			cv_elements = self.detector.detect_from_image(screenshot_b64, scale_factor, detect_sheets)
